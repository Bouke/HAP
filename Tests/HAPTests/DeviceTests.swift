...
class DeviceTests: XCTestCase {
    func testInstanceIdentifiers() {
        ...
        do {
            ...
            let services = accessories.flatMap({ $0.services })
            let characteristics = services.flatMap({ $0.characteristics })
            XCTAssertEqual(accessories.map({$0.aid}), [1, 2])
            XCTAssertEqual(services.map({$0.iid}), [1, 8, 1, 8])
            XCTAssertEqual(services.filter({$0.type == .info}).map({$0.iid}), [1, 1])
            XCTAssertEqual(characteristics.map({$0.iid}),
                           [2, 3, 4, 5, 6, 7, 9, 10, 11, 12, 13, 2, 3, 4, 5, 6, 7, 9, 10, 11, 12])
        }
    }
}
