// swiftlint:disable force_cast inclusive_language file_length
import Foundation
...
func inspect(source plistPath: URL, target outputPath: String) throws {
    ...
    try FileManager.default.createDirectory(atPath: "\(outputPath)/Enums",
                                            withIntermediateDirectories: false)
    try FileManager.default.createDirectory(atPath: "\(outputPath)/Services",
                                            withIntermediateDirectories: false)
    try FileManager.default.createDirectory(atPath: "\(outputPath)/Characteristics",
                                            withIntermediateDirectories: false)
    ...
    for (name, dict) in services {
        guard let title = dict["DefaultDescription"] as? String,
              let id = dict["ShortUUID"] as? String else {
            continue
        }
    ...
    try writeToFile(atPath: "ServiceType.swift") { write in
        ...
        for service in serviceInfo.sorted(by: { $0.className < $1.className }) {
            write("    static let \(serviceName(service)) = ServiceType(0x\(service.id.suffix(4)))")
        }
        write("""
        }
    ...
    for (name, dict) in characteristics {
        if let title = dict["DefaultDescription"] as? String,
            ...
            if !blacklistCharacteristics.contains(name) && whitelistedCharacteristics.contains(name) {
    ...
    try writeToFile(atPath: "CharacteristicType.swift") { write in
        ...
        for characteristic in characteristicInfo.sorted(by: { $0.title < $1.title }) {
            let type = "0x\(characteristic.id.suffix(4))"
            write("    static let \(serviceName(characteristic)) = CharacteristicType(\(type))")
        }
        write("""
        }
        ...
        for characteristic in characteristicInfo.sorted(by: { $0.title < $1.title }) {
            if let id = Int(characteristic.id, radix: 16) {
                _ = String(id, radix: 16, uppercase: true)
                write("        case .\(serviceName(characteristic)): return \"\(characteristic.title)\"")
            }
        }
        write("""
                case let .appleDefined(typeCode):
    ...
    for (name, dict) in units where (dict["DefaultDescription"] as? String) != nil {
    ...
    for defaultType in defaultTypes {
        if enums.contains(where: { $0.info.hkname == defaultType.characteristic }) {
            // Already defined
    ...
    for (enumName, type, values, info) in enums.sorted(by: { $0.enumName < $1.enumName }) {
        if let enumCases = determineEnumerationCases(values) {
            try writeEnumeration(enumName: enumName,
                                 type: type,
                                 values: enumCases,
                                 max: info.maxValue)
            enumeratedCharacteristics.insert(info.hkname)
            defaultEnumCase[info.hkname] = (enumCases.allKeys[0] as! String).parameterName()
        } else if let defaultType = defaultTypes.first(where: { $0.characteristic == info.hkname }) {
            ...
            try writeEnumeration(enumName: defaultType.enumName,
                                 type: defaultType.baseType,
                                 values: valuedict,
                                 max: info.maxValue)
            enumeratedCharacteristics.insert(info.hkname)
        ...
        try writeToFile(atPath: "Services/Service.\(service.className).swift") { write in
            ...
            for characteristic in service.required {
                if let info = characteristicInfo.first(where: { $0.hkname == characteristic }) {
                    let name = info.title.parameterName()
                    let characteristiceType = ".\(serviceName(info))"
                    write("""
                                    \(name) = getOrCreateAppend(
                                        type: \(characteristiceType),
                                        characteristics: &unwrapped,
                                        generator: { PredefinedCharacteristic.\(serviceName(info))() })
                        """)
                }
            }
            ...
            write("""
                            super.init(type: .\(serviceName(service)), characteristics: unwrapped)
                        }
    ...
    func defaultValue(_ characteristic: CharacteristicInfo) -> String {
        if characteristic.permissions.contains(.read) {
            if enumeratedCharacteristics.contains(characteristic.hkname) {
                ...
                case "String": return "\"\""
                case "Float", "Int", "UInt8", "UInt16", "UInt32":
                    return characteristic.minValue?.stringValue ?? "0"
    ...
    for characteristic in characteristicInfo.sorted(by: { $0.title < $1.title }) {
        ...
        let path = "Characteristics/Characteristic.\(name.uppercasedFirstLetter()).swift"

        try writeToFile(atPath: path) { write in
            let unit = characteristic.units != nil ? ".\(unitName(characteristic.units!))" : "nil"
            func writeFactoryArgumentsWithDefaults() {
                ...
                write("        unit: CharacteristicUnit? = \(unit),")
...
func serviceName(_ service: ServiceInfo) -> String {
    serviceName(service.title, uuid: service.id)
}

func serviceName(_ info: CharacteristicInfo) -> String {
    serviceName(info.title, uuid: info.id)
}
...
func determineEnumerationCases(_ values: NSDictionary) -> NSDictionary? {
    // Check if keys are digits
    if values.allKeys.allSatisfy({ ($0 as? NSNumber)?.intValue ?? Int($0 as? String ?? "x") == nil }) {
        return values
    }

    // Check if values are also all digits
    guard values.allValues.allSatisfy({ ($0 as? NSNumber)?.intValue ?? Int($0 as? String ?? "x") == nil }) else {
        return nil
    }
    ...
    return NSDictionary(objects: keys, forKeys: vals as! [NSCopying])
}
