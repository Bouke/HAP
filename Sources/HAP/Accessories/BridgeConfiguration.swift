...
extension Service {
    open class BridgeConfiguration: Service {
        public let configureBridgedAccessoryStatus = GenericCharacteristic<ConfigureBridgedAccessoryStatus>(
            type: .configureBridgedAccessoryStatus,
            permissions: [.read, .events])
        public let discoverBridgedAccessories = GenericCharacteristic<DiscoverBridgedAccessories>(
            type: .discoverBridgedAccessories,
            permissions: [.read, .events])
        public let discoveredBridgedAccessories = GenericCharacteristic<DiscoveredBridgedAccessories>(
            type: .discoveredBridgedAccessories,
            permissions: [.read, .events])
        public let configureBridgedAccessory = GenericCharacteristic<ConfigureBridgedAccessory>(
            type: .configureBridgedAccessory,
            permissions: [.read, .events])

        public init() {
            super.init(type: .bridgeConfiguration,
                       characteristics: [configureBridgedAccessoryStatus,
